...
def parseTxtLine(data_line: str) -> list:
    ...
    if len(char_data) == 11: # For example: My Unit, Class = Mage
        cl_idx = 2
        if char_data[0] == 'My': # My Unit case
            clean_data[0] = char_data[0] + char_data[1]
            clean_data[1] = char_data[2]
            start_idx = 3 
    ...
    if len(char_data) == 13: # For example: Class = "Dark Mage / Sorceror"
        ...
        if char_data[0] == 'My': # My Unit case
            clean_data[0] = char_data[0] + char_data[1]
            clean_data[1] = char_data[2] + char_data[3] + char_data[4]
        else:
            clean_data[1] = char_data[1].strip('\t') + char_data[2] + char_data[3] + char_data[4]

    if len(char_data) == 14: # For example: 'My Unit - Dark Mage / Sorceror ...'
        start_idx = 6
        cl_idx = 2
        clean_data[0] = char_data[0] + char_data[1]
        clean_data[1] = char_data[2].strip('\t')
        for i in range(3,6):
            clean_data[1] += char_data[i]

    if len(char_data) == 15: # For Example: Cecil - Peg / Falco / Draco Knight ...'
        start_idx = 7
        cl_idx = 2
        clean_data[0] = char_data[0]
        clean_data[1] = char_data[1].strip('\t')
        for i in range(2,7):
            clean_data[1] += char_data[i]

    if len(char_data) == 16: # For example: 'My Unit - Peg / Falco / Draco Knight ...'
        start_idx = 8
        cl_idx = 2
        clean_data[0] = char_data[0] + char_data[1]
        clean_data[1] = char_data[2].strip('\t')
